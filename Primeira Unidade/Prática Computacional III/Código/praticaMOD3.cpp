...
class RushHour {
    ...
    void initFree(State* s) {
        ...
        for (unsigned int i = 0; i < nbcars; i++) {
            if (horiz[i])
                for (int ocupado = 0; ocupado < len[i]; ocupado++)
                    free[moveon[i]][s->pos[i] + ocupado] = false;
            else
                for (int ocupado = 0; ocupado < len[i]; ocupado++)
                    free[s->pos[i] + ocupado ][moveon[i] ] = false;
        }
    }
    ...
    void printSolution(State* s) {
        ...
        for (s = solve(s); s->prev != NULL; s = s->prev){
            if(horiz[s->c]){
                if(s->d==1) cout << "Veiculo " << color[s->c] << " para direita \n";
                else cout << "Veiculo " << color[s->c] << " para esquerda \n";
            }
            else{
                if(s->d==1) cout << "Veiculo " << color[s->c] << " para cima \n";
                else cout << "Veiculo " << color[s->c] << " para baixo \n";
            }
        }
    }
